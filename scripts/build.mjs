...
import { releaseRepository, releaseManifest } from './release-config.mjs';

const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const buildRoot = path.join(projectRoot, 'build');
const packageRoot = path.join(buildRoot, 'package');
// Validate before touching build outputs. Source manifests intentionally have no
// update URL until the maintainer supplies the community repository.
const repository = releaseRepository();
const manifest = releaseManifest(JSON.parse(await readFile(path.join(projectRoot, 'manifest.json'), 'utf8')), repository);
...
const xpiName = `mktero-community-${manifest.version}.xpi`;
...
await Promise.all([
    writeFile(path.join(packageRoot, 'manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`),
    copyText('LICENSE', 'LICENSE'),
    copyText('NOTICE.md', 'NOTICE.md'),
    ...
    addons: {
        [manifest.applications.zotero.id]: {
            updates: [{
                ...
                update_link: `https://github.com/${repository}/releases/download/v${manifest.version}/${xpiName}`,
