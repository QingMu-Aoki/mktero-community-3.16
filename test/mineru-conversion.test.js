...
test('cancelled collection retains account binding and rejects a different account on resume', async () => {
    let saved; let submissions = 0; let collections = 0;
    const pendingTasks = { get: async () => saved, put: async (key, task) => { saved = task; }, delete: async () => { saved = null; } };
    const conversion = new MinerUConversion({ pendingTasks, client: {
        submit: async () => { submissions++; return { batchID: 'batch', dataID: 'data', fileName: 'a.pdf' }; },
        collect: async () => { collections++; throw new Error('interrupted'); },
    } });
    await assert.rejects(conversion.convert({ key: CONVERSION_KEY, apiKey: 'key-one', accountID: 'b'.repeat(64) }));
    assert.equal(saved.accountID, 'b'.repeat(64));
    await assert.rejects(conversion.convert({ key: CONVERSION_KEY, apiKey: 'key-two', accountID: 'c'.repeat(64) }), { code: 'MINERU_ACCOUNTS_PAUSED' });
    assert.equal(submissions, 1); assert.equal(collections, 1);
});
