...
test('batch menu expands parent, freezes command selection, exposes stop and cleans up', async () => {
    const parent = regularItem(10, [41, 42]);
    const pdfs = [pdfItem(41), pdfItem(42)];
    const harness = createMenuHarness([parent, pdfs[0]]);
    const calls = []; let running = false; let stopped = 0;
    const dispose = registerItemContextMenu({
        zotero: { Items: { get: id => pdfs.find(pdf => pdf.id === id) } }, window: harness.window,
        onOpen: () => {}, onBatch: ids => calls.push(ids), onStopBatch: () => stopped++,
        isBatchRunning: () => running, onError: error => { throw error; },
    });
    showMenu(harness.document);
    const batch = harness.document.getElementById('mktero-batch-convert');
    assert.match(batch.getAttribute('label'), /2/);
    batch.dispatchEvent(new harness.document.defaultView.Event('command'));
    harness.select([pdfs[1]]);
    await Promise.resolve();
    assert.deepEqual(calls, [[41, 42]]);
    running = true; showMenu(harness.document);
    assert.equal(batch.disabled, true);
    const stop = harness.document.getElementById('mktero-batch-stop');
    assert.equal(stop.hidden, false);
    stop.dispatchEvent(new harness.document.defaultView.Event('command'));
    assert.equal(stopped, 1); dispose();
    assert.equal(harness.document.getElementById('mktero-batch-stop'), null);
});

test('shows ready state and imports selected PDF without opening the PDF reader', async () => {
    const harness = createMenuHarness([pdfItem(42)]);
    const imported = [];
    const dispose = registerItemContextMenu({
        zotero: {}, window: harness.window,
        onOpen: () => {}, onImport: id => imported.push(id),
        isReady: () => true, onError: error => { throw error; },
    });
    showMenu(harness.document);
    assert.equal(harness.document.querySelector('#mktero-read-as-markdown').getAttribute('label'), 'Open Markdown');
    const item = harness.document.querySelector('#mktero-import-local');
    assert.equal(item.hidden, false);
    item.dispatchEvent(new harness.document.defaultView.Event('command'));
    await Promise.resolve();
    await Promise.resolve();
    assert.deepEqual(imported, [42]);
    dispose();
    assert.equal(harness.document.querySelector('#mktero-import-local'), null);
});
