...
test('persists the account fingerprint without API credentials', async t => {
    const rootPath = await mkdtemp(path.join(os.tmpdir(), 'mktero-account-task-'));
    t.after(() => rm(rootPath, { recursive: true, force: true }));
    const options = { rootPath, ioUtils: createNodeIOUtils(), pathUtils: createNodePathUtils() };
    const task = { batchID: 'batch', dataID: 'data', uploadedAt: Date.now(), accountID: 'd'.repeat(64), apiKey: 'must-not-be-stored' };
    await new MinerUPendingTaskStore(options).put(CONVERSION_KEY, task);
    const restored = await new MinerUPendingTaskStore(options).get(CONVERSION_KEY);
    assert.equal(restored.accountID, task.accountID);
    assert.equal(restored.apiKey, undefined);
    assert.doesNotMatch(await readFile(path.join(rootPath, 'tasks', `${CONVERSION_KEY}.json`), 'utf8'), /must-not-be-stored/);
});
