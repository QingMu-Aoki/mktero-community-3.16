...
test('builds reproducible release assets and Zotero update metadata', async () => {
    ...
    assert.match(strFromU8(packageEntries['LICENSE']), /Copyright \(c\) 2026 Tony/);
    assert.match(strFromU8(packageEntries['LICENSE']), /Copyright \(c\) 2026 青木 · Aoki/);
    assert.equal(JSON.parse(strFromU8(packageEntries['manifest.json'])).applications.zotero.update_url, 'https://github.com/test-fixture/mktero-community/releases/latest/download/updates.json');
    assert.deepEqual(updates, {
        addons: {
            [manifest.applications.zotero.id]: {
                updates: [{
                    version: manifest.version,
                    update_link: `https://github.com/test-fixture/mktero-community/releases/download/v${manifest.version}/${xpiName}`,
...
async function buildProject() {
    await execFileAsync(process.execPath, ['scripts/build.mjs'], {
        cwd: projectRoot,
        env: { ...process.env, GITHUB_ACTIONS: 'false', MKTERO_RELEASE_REPOSITORY: 'test-fixture/mktero-community' },
    });
}
