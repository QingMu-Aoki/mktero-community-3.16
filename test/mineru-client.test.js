...
test('submission retries explicit throttling with the same key, but not ambiguous server errors', async () => {
    const keys = []; const waits = []; let calls = 0;
    const client = new MinerUClient({ sleep: async ms => waits.push(ms), fetch: async (url, options) => {
        keys.push(options.headers?.Authorization);
        if (++calls === 1) return jsonResponse({}, 429, { get: () => '2' });
        if (calls === 2) return jsonResponse({ code: 0, data: { batch_id: 'batch', file_urls: ['https://upload.example/file'] } });
        return { ok: true, status: 200 };
    } });
    const limited = []; client.onRateLimit = (...args) => limited.push(args);
    await client.submit({ apiKey: 'test-key', fileName: 'a.pdf', fileData: new Uint8Array([1]), dataID: 'id' });
    assert.deepEqual(keys.slice(0, 2), ['Bearer test-key', 'Bearer test-key']);
    assert.deepEqual(waits, [2000]); assert.deepEqual(limited, [['test-key', 2000]]);
    let failedCalls = 0;
    const failing = new MinerUClient({ fetch: async () => { failedCalls++; return jsonResponse({ code: -1 }, 500); } });
    await assert.rejects(failing.submit({ apiKey: 'test-key', fileName: 'a.pdf', fileData: new Uint8Array([1]) }));
