...
export class MinerUClient {
    ...
    async submit({
        ...
        const batch = await this.#submitJSON(`${this.apiBase}/file-urls/batch`, {
            method: 'POST',
    ...
    async #requestJSON(url, options) {
        await this.beforeAccountRequest?.(String(options.headers?.Authorization || '').replace(/^Bearer /, ''), options.signal);
        return this.#runRequest({
            ...
            operation: async requestSignal => {
                ...
                if (response?.status === 429) {
                    ...
                    this.onRateLimit?.(String(options.headers?.Authorization || '').replace(/^Bearer /, ''), delay);
                    throw httpError('MinerU request failed', response);
                }
                let payload;
                try {
    ...
    async #submitJSON(url, options) {
        // Only explicit throttling is safe to retry. Network/5xx failures may
        // already have created a task, so never blindly repeat a submission.
        for (let attempt = 0; ; attempt++) {
            try { return await this.#requestJSON(url, options); }
            catch (error) {
                if (error.status !== 429 || attempt >= 2 || options.signal?.aborted) throw error;
                await waitFor(this.sleep, error.retryAfterMs ?? Math.min(60000, 2000 * (2 ** attempt)), options.signal);
            }
        }
    }
...
function apiError(code, message) {
    ...
    if (/quota|额度|配额|每日.*上限/i.test(String(message))) {
        const error = new Error('MinerU account quota exceeded');
        error.code = 'MINERU_QUOTA_EXCEEDED';
        return error;
    }
