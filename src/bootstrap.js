...
import { getMinerUAccounts } from './config/mineru-accounts.js';
import { MinerUAccountPool } from './mineru/account-pool.js';
import { LibraryMarkdown } from './platform/library-markdown.js';
import { equivalentPDFLayouts } from './pdf/pdf-layout-equivalence.js';
import { LOCAL_PARSER_PROFILE } from './mineru/local-result.js';
import {
...
globalThis.startup = async function startup({ id, rootURI }) {
    ...
    const mineruClient = new MinerUClient({ createAbortController: createZoteroAbortController });
    ...
    runtime.accountPool = new MinerUAccountPool({ conversion, pendingTasks, getAccounts: () => getMinerUAccounts(Zotero), createAbortController: createZoteroAbortController });
    mineruClient.onRateLimit = (token, delay) => runtime.accountPool?.rateLimited(token, delay);
    mineruClient.beforeAccountRequest = (token, signal) => runtime.accountPool?.waitForAccount(token, signal);
    const mineruExtractor = new MinerUDocumentExtractor({
        zotero: Zotero,
        conversion: runtime.accountPool,
    ...
    runtime.libraryMarkdown = new LibraryMarkdown({
        zotero: Zotero, io: IOUtils, path: PathUtils, cache,
        snapshotStore: runtime.savedMarkdownStore,
        createAbortController: createZoteroAbortController,
        accountPool: runtime.accountPool,
        getProvider: () => getConversionProvider(Zotero),
        comparePDFs: async (left, right) => {
            ...
            const engines = runtime.localComparisonEngines ||= new Set();
            engines.add(engine);
            try {
                return equivalentPDFLayouts(await engine.extract(left), await engine.extract(right));
            }
            finally {
        ...
        translate: runtimeTranslate, createFilePicker: createZoteroFilePicker,
        preparePDFIndex: (id, options) => {
            void preparePDFIndexForItem(id, options, pdfAnnotationLocator).catch(error => Zotero.logError(error));
        },
    });
    await runtime.libraryMarkdown.register(runtime.id);
    runtime.service = new MarkdownDocumentService({
        extractor: runtime.libraryMarkdown.wrap(extractor),
        annotationOverlay,
        localAnnotations,
    ...
    runtime.disposeCacheObserver = observeLocalCacheCleared(
        ...
        () => {
            ...
            runtime.libraryMarkdown?.invalidate();
        }
    );
...
globalThis.shutdown = function shutdown() {
    for (const engine of runtime.localComparisonEngines || []) void engine.dispose();
    runtime.localComparisonEngines?.clear();
    runtime.libraryMarkdown?.dispose();
    runtime.accountPool?.dispose();
    runtime.accountPool = null;
    runtime.libraryMarkdown = null;
    abortAllConversions();
...
function registerMainWindowContextMenu(window) {
    ...
    const dispose = registerItemContextMenu({
        ...
        isReady: id => runtime.libraryMarkdown?.ready(id),
        onBatch: ids => runtime.libraryMarkdown.startBatch(ids),
        onStopBatch: () => runtime.libraryMarkdown.stopBatch(),
        isBatchRunning: () => Boolean(runtime.libraryMarkdown?.batch),
        onImport: async id => {
            try {
                if (await runtime.libraryMarkdown.import(id)) {
                    runtime.presenter?.closeForSourceItem?.(id);
                    await openItemAsMarkdown(id);
                }
            }
            catch (error) {
                Zotero.logError(error);
                Zotero.getMainWindow?.()?.alert?.(runtimeTranslate('library.importError') + '\n' + String(error?.message || error));
            }
        },
