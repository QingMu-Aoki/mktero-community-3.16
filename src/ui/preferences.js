...
import { mountMinerUAccounts } from './mineru-account-settings.js';
...
export function createPreferencesController({
    ...
    function updateConversionApiKeyControl() {
        ...
        if (!accountSettings) accountSettings = mountMinerUAccounts({
            document, zotero, container: conversionApiKeyInput.parentNode,
            translate: key => localization.t(key),
        });
        const mineru = getSelectedConversionProvider() === CONVERSION_PROVIDER_MINERU;
        accountSettings.hidden = !mineru;
        conversionApiKeyInput.hidden = mineru;
