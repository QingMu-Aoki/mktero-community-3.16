...
export function extractMarkdownAssetOutline(markdown, chromeRanges = []) {
    ...
    const items = [];
    // Match spaced OCR captions in a same-length view; never rewrite stored
    // Markdown or shift source-map offsets.
    const figureSource = source.replace(
        /^F[ \t]*I[ \t]*G[ \t]*U[ \t]*R[ \t]*E[ \t]+(\d+[A-Za-z]?)[ \t]+/gm,
        (prefix, number) => {
            const normalized = `Figure ${number}. `;
            return normalized.length <= prefix.length
                ? normalized.padEnd(prefix.length, ' ') : prefix;
        }
    );
    for (const group of findAcademicFigures(figureSource)) {
